  ...
  return (builder) => {
    ...
    for (const node of nodes) {
      ...
      for (const column of Object.keys(databaseJson)) {
        ...
        console.log(" createInputColumnSelector", column, selects)
        const selection =
...
function createManyToManyExtraSelectionIfNeeded(builder, column, node) {
  if (node.parentEdge && node.parentEdge.relation.isObjectionManyToManyRelation) {
    ...
    console.log("createManyToManyExtraSelectionIfNeeded relation", relation)
    console.log("relation.joinTableExtras", relation.joinTableExtras)
